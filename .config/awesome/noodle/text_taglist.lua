  ...
  local function update_widget()
    for i = 1, ntags do
      ...
      if s.tags[i] and s.tags[i].selected then
          tag_text[i].markup = helpers.colorize_text(tf[i], cf)
      elseif s.tags[i] and s.tags[i].urgent then
          tag_text[i].markup = helpers.colorize_text(tu[i], cu)
      elseif tag_clients and #tag_clients > 0 then
          tag_text[i].markup = helpers.colorize_text(to[i], co)
      else
          tag_text[i].markup = helpers.colorize_text(te[i], ce)
      end
    end
  end
